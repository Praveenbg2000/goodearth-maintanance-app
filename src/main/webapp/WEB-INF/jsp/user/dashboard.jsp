...
html, body {
  scroll-behavior: auto !important; /* Disable smooth scroll which causes jumps */
  overscroll-behavior-y: contain; /* Prevents "bounce" on mobile */
}

/* Ensure clicking labels/inputs doesn't trigger a page jump */
              ...
              <div class="discription-with-content">
                <div class="discription-image">
                  <c:if test="${not empty task.taskImage}">
                    <img src="${pageContext.request.contextPath}/images/${task.taskImage}" style="width: 120px; height: 120px; border-radius: 8px;">
                  </c:if>
                </div>
                <div style="width: 100%;">
  ...
  function initializeAccordionsAndCheckboxes() {
    ...
    accordions.forEach(acc => {
      ...
      header.addEventListener("click", (e) => {
    	  // 1. Allow checkbox clicks without toggling accordion
    	  if (e.target.classList.contains('task-check') || e.target.closest('.custom-check')) {
    	    e.stopPropagation();
    	    return;
    	  }

    	  // Prevent browser focus scroll
    	  e.preventDefault();

    	  const isOpen = acc.classList.contains("open");
    	  const content = acc.querySelector(".accordion-content");

    	  // Save header position before toggle
    	  ...
    	  document.querySelectorAll(".accordion").forEach(a => {
    	    if (a !== acc) {
    	      ...
    	      const otherIcon = a.querySelector(".material-icons");
    	  ...
    	  // Restore scroll position → forces downward opening
    	  const newHeaderTop = header.getBoundingClientRect().top;
    	  window.scrollBy(0, newHeaderTop - headerTop);
    	});
